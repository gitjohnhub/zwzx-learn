...
import { generate_download_link, formatDate, add_underLine, add_p,add_qianzi,add_date ,add_title} from '@/utils/utils';
...
function generate_guquanzhuanrang_xieyi() {
  const all_content = add_title('股权转让协议')+
      add_p(`本协议由以下各方于${add_underLine(formatDate(formState.decide_date))}在上海共同签署。`)+
      add_p(`出让方:${add_underLine(formState.origin_gudong_name)}(以下称甲方）住所：${add_underLine(formState.origin_gudong_address)}`)+
      add_p(`受让方:${add_underLine(formState.modify_gudong_name)}(以下称乙方）住所：${add_underLine(formState.modify_gudong_address)}`)+
      add_p(
    ...
    add_p(`第二条 （承诺和保证）`)+
    add_p(`甲方保证本合同第一条转让给乙方的股权为甲方合法拥有，甲方拥有完全、有效的处分权。甲方保证其所转让的股权没有设置任何质押或其他担保权，不受任何第三人的追索。`)+
    add_p(`第三条 （违约责任）`)+
    add_p(`${add_underLine("违约责任") }`)+
    add_p(`第四条 （解决争议的方法）`)+
    add_p(`本协议受中华人民共和国相关法律的羁束并适用其解释。`)+
    add_p(`凡因本协议引起的或与本协议有关的任何争议，双方应友好协商解决。协商不成，应直接向人民法院起诉。`)+
    add_p(`第五条 （其他）`)+
